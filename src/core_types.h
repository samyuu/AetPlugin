...
constexpr FileAddr operator+(FileAddr left, FileAddr right) { return static_cast<FileAddr>(static_cast<i64>(left) + static_cast<i64>(right)); }
constexpr FileAddr operator-(FileAddr left, FileAddr right) { return static_cast<FileAddr>(static_cast<i64>(left) - static_cast<i64>(right)); }
constexpr FileAddr& operator+=(FileAddr& left, FileAddr right) { return (left = static_cast<FileAddr>(static_cast<i64>(left) + static_cast<i64>(right))); }
constexpr FileAddr& operator-=(FileAddr& left, FileAddr right) { return (left = static_cast<FileAddr>(static_cast<i64>(left) - static_cast<i64>(right))); }
...
inline auto FindIfOrNull(CollectionType& collection, Func pred) -> typename CollectionType::value_type*
{
	for (auto it = collection.begin(); it != collection.end(); it++) { if (pred(*it)) return &(*it); } return nullptr;
}

template <typename CollectionType, typename Func>
inline auto FindIfOrNull(const CollectionType& collection, Func pred) -> const typename CollectionType::value_type*
{
	for (auto it = collection.begin(); it != collection.end(); it++) { if (pred(*it)) return &(*it); } return nullptr;
}

template <typename CollectionType, typename Func>
inline size_t FindIndexOf(CollectionType& collection, Func pred)
{
	size_t i = 0; for (auto it = collection.begin(); it != collection.end(); it++) { if (pred(*it)) { return i; } i++; } return i;
}

template <typename CollectionType, typename Func>
inline size_t FindLastIndexOf(CollectionType& collection, Func pred)
{
	size_t size = collection.size(), i = (size == 0) ? 0 : (size - 1);
	for (auto it = collection.rbegin(); it != collection.rend(); it++) { if (pred(*it)) { return i; } i--; } return i;
}
...
struct ivec4
{
	...
	ivec4() = default;
	explicit constexpr ivec4(i32 scalar) : x(scalar), y(scalar), z(scalar), w(scalar) {}
	constexpr ivec4(i32 x, i32 y, i32 z, i32 w) : x(x), y(y), z(z), w(w) {}
	constexpr ivec4(ivec2 a, ivec2 b) : x(a.x), y(a.y), z(b.x), w(b.y) {}
	constexpr ivec4(const ivec4& other) : x(other.x), y(other.y), z(other.z), w(other.w) {}
...
struct vec4
{
	...
	explicit constexpr vec4(const ivec4& other) : x(static_cast<f32>(other.x)), y(static_cast<f32>(other.y)), z(static_cast<f32>(other.z)), w(static_cast<f32>(other.w)) {}
...
struct mat3
{
	...
	inline vec3& operator[](size_t index) { return V[index]; }
	inline vec3 operator[](size_t index) const { return V[index]; }
};
...
inline f32 ByteSwapF32(f32 value) { u32 result = ByteSwapU32(*reinterpret_cast<u32*>(&value)); return *reinterpret_cast<f32*>(&result); }
inline f64 ByteSwapF64(f64 value) { u64 result = ByteSwapU64(*reinterpret_cast<u64*>(&value)); return *reinterpret_cast<f64*>(&result); }
