...
namespace Path
{
	...
	{
		pathA.reserve(pathA.size() + sizeof(DirectorySeparator) + pathB.size());
		pathA += DirectorySeparator;
		pathA += pathB;
		return pathA;
	}
	...
	{
		return Combine(std::string { pathA }, pathB);
	}
}
...
namespace Directory
{
	...
	{
		std::wstring directoryW = UTF8::Widen(directoryPath);
		for (wchar_t& c : directoryW)
			c = (c == L'/') ? L'\\' : c;
		if (directoryW.back() != L'\\')
			directoryW += L'\\';
		directoryW += L"*";

		WIN32_FIND_DATAW findData = {};
		const HANDLE findResult = ::FindFirstFileW(directoryW.c_str(), &findData);
		...
		do
		{
			...
			{
				fullPathBuffer = directoryPath;
				fullPathBuffer += '/';
				fullPathBuffer += UTF8::Narrow(findData.cFileName);
				if (forEachFileFunc(fullPathBuffer) == ControlFlow::Break)
					break;
			}
		}
